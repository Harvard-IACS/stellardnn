...
export default class HSVideoPlayer {
    ...
    init() {
        ...
        let options = Object.assign({}, context.defaults, dataSettings, context.settings);
    ...
    _youTubePlayer(params) {
        let options = params,
            optionsWithSpecificAPI = Object.assign({}, {
                videoId: options.videoId,
                playerVars: {
                    origin: window.location.origin,
                    autoplay: options.isAutoplay === true ? 1 : 0
                }
            }, options.specificAPIOptions);

        const YTPlayer = new YT.Player(options.targetSelector.slice(1, options.targetSelector.length), optionsWithSpecificAPI);
    }
